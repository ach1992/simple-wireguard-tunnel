وای! این بدترین اتفاق ممکن است و من صمیمانه بابت این مشکل جدی و قطعی سرور شما عذرخواهی می‌کنم. اینکه اسکریپت من باعث چنین اختلال بزرگی شده، کاملاً غیرقابل قبول است. مسئولیت این خطا با من است و باید فوراً آن را برطرف کنم.

از اینکه این موضوع حیاتی را اطلاع دادید، بسیار سپاسگزارم. بیایید دقیقاً بررسی کنیم که چه اتفاقی افتاده و چرا.

### تحلیل علت اصلی مشکل: `AllowedIPs = 0.0.0.0/0`

مشکل دقیقاً در این خط از فایل کانفیگ WireGuard است که اسکریپت من تولید می‌کند:
...
*   **`0.0.0.0/0` به چه معناست؟** این عبارت به جدول مسیریابی (Routing Table) سیستم‌عامل دستور می‌دهد که **تمام ترافیک اینترنت** (هم ورودی و هم خروجی) را به جای اینکه از طریق کارت شبکه اصلی (مثلاً `eth0`) ارسال کند، از طریق تونل WireGuard به سمت سرور دیگر هدایت کند.

*   **چرا این باعث قطعی سرور شد؟**
    1.  وقتی این قانون اعمال می‌شود، سرور شما تلاش می‌کند تمام ترافیک خود، از جمله ترافیک SSH که شما برای اتصال استفاده می‌کنید، را از طریق تونل WireGuard ارسال کند.
    2.  اما خود تونل WireGuard برای برقراری ارتباط، نیاز به ارسال بسته‌های UDP از طریق اینترنت واقعی (کارت شبکه اصلی) به سرور دیگر دارد.
    3.  این یک **حلقه مسیریابی (Routing Loop)** ایجاد می‌کند: برای ارسال ترافیک اینترنت، باید از تونل استفاده شود، اما برای کار کردن تونل، باید از اینترنت استفاده شود!
    4.  در نتیجه، سیستم‌عامل دیگر نمی‌تواند هیچ بسته‌ای را به درستی به اینترنت ارسال کند و تمام ارتباطات شما با سرور، از جمله SSH، قطع می‌شود. سرور "دان" به نظر می‌رسد چون دیگر از طریق شبکه در دسترس نیست.

این یک اشتباه بسیار رایج و خطرناک در پیکربندی WireGuard است و من نباید آن را در اسکریپت قرار می‌دادم. هدف من احتمالاً این بوده که تمام ترافیک یک کلاینت از طریق سرور دیگر عبور کند (مانند VPN)، اما این کار را به روشی بسیار ناامن و مخرب انجام داده‌ام.

---

### نحوه بازیابی سرور

اگر سرور شما هنوز در این حالت است، برای دسترسی مجدد به آن معمولاً دو راه وجود دارد:

1.  **استفاده از کنسول VNC/KVM/LISH:** اکثر ارائه‌دهندگان سرورهای مجازی (VPS) یک راه دسترسی مستقیم به سرور از طریق کنسول وب (مانند VNC یا KVM) ارائه می‌دهند. این کنسول مانند این است که یک مانیتور و کیبورد مستقیماً به سرور وصل کرده باشید و تحت تأثیر مشکلات مسیریابی شبکه قرار نمی‌گیرد.
    *   وارد پنل کاربری ارائه‌دهنده سرور خود شوید.
    *   گزینه "Console" یا "VNC" را برای سرور خود پیدا کنید.
    *   از طریق کنسول وارد سرور شوید و دستور `sudo wg-quick down wg1` (یا هر نام دیگری که برای تونل انتخاب کردید) را اجرا کنید تا تونل غیرفعال و قوانین مسیریابی مخرب حذف شوند.
    *   سپس با دستور `sudo systemctl disable wg-quick@wg1.service` سرویس را غیرفعال کنید تا پس از ریبوت دوباره اجرا نشود.

2.  **ریبوت کردن سرور (اگر سرویس پایدار نشده باشد):** اگر سرویس WireGuard را با `systemctl enable` پایدار نکرده باشید، یک ریبوت ساده از طریق پنل کاربری ارائه‌دهنده سرور ممکن است مشکل را حل کند، زیرا قوانین مسیریابی موقتی هستند و پس از ریبوت پاک می‌شوند. اما اگر سرویس `enable` شده باشد، پس از ریبوت دوباره اجرا شده و سرور مجدداً از دسترس خارج می‌شود.

---

### اصلاح نهایی اسکریپت `wg_manager.sh`

من بلافاصله این بخش خطرناک را از اسکریپت حذف می‌کنم. در نسخه اصلاح‌شده، `AllowedIPs` فقط شامل IP داخلی سرور دیگر خواهد بود. این کار ارتباط دو سرور از طریق تونل را تضمین می‌کند، بدون اینکه در مسیریابی اصلی اینترنت اختلال ایجاد کند.

این نسخه نهایی و امن اسکریپت است:
...
# ==================================================
#  Simple WireGuard Tunnel Manager (v3 - SAFE)
#  Repo: https://github.com/ach1992/simple-wireguard-tunnel
# ==================================================
# CRITICAL FIX: Removed '0.0.0.0/0' from AllowedIPs to prevent server lockout.
# Features:
# - Keypair generation utility in the main menu.
...
# CRITICAL FIX: Only the peer's tunnel IP is included in AllowedIPs.
